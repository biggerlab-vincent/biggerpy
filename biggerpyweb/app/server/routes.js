...
// view & delete accounts //
	...
	app.get('/manage_template', function(req, res) {
	...
	app.post('/delete_template', function (req, res) {
		TemplateManager.deleteTemplate(req.body["id"], function (e, o) {
			if (!e) {
				res.status(200).send('ok');
				//res.redirect('/manage_template');
			} else {
	...
	app.post('/update_template', function (req, res) {
		if (req.session.user == null || req.session.user.name !== "biggerlab_admin") {
			...
			TemplateManager.updateTemplate(req.body['id'],req.body["lesson"], req.body["title"], req.body["content"], req.body["contentFinal"], function (e, o) {
				if (!e) {
					res.status(200).send('ok');
					//res.redirect('/manage_template');					
				} else {
	...
	//manage template load
	app.post('/manageTemplate', function (req, res) {
		if (req.session.user == null || req.session.user.name !== "biggerlab_admin") {
			// USER IS NOT ALLOWED
		} else {
			...
			TemplateManager.loadManageTemplate(req.body['id'], function (e, o) {
				if (e) {
					res.status(400).send(null);
				} else {
					// Loaded
    ...
    app.post('/load_template', function(req, res) {
        if (req.session.user == null || req.session.user.name !== "biggerlab_admin") {
            // USER IS NOT ALLOWED
        } else {
            TemplateManager.loadTemplate( function(e, TemplateManager){
                res.render('templatelist', { title : 'template List', accts : templates });
            })
	...
	app.post('/template-list', function (req, res) {
		if (req.session.user == null) {
			res.redirect('/');
		} else {
			TemplateManager.updateAccount({
				id: req.session.user._id,
				name: req.body['name'],
				email: req.body['email'],
				pass: req.body['pass'],
